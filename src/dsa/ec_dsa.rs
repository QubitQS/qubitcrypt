...
impl Dsa for EcDsaManager {
    ...
    fn key_gen(&mut self) -> Result<(Vec<u8>, Vec<u8>)> {
        ...
        let result = result.map_err(|_| QubitCryptError::KeyPairGenerationFailed)?;
        Ok(result)
    }
    ...
    fn sign(&self, sk: &[u8], msg: &[u8]) -> Result<Vec<u8>> {
        ...
        let result = result.map_err(|_| QubitCryptError::SignatureFailed)?;
        Ok(result)
    }
    ...
    fn verify(&self, pk: &[u8], msg: &[u8], signature: &[u8]) -> Result<bool> {
        ...
        let result = result.map_err(|_| QubitCryptError::SignatureVerificationFailed)?;
        Ok(result)
    }
    ...
    fn get_public_key(&self, sk: &[u8]) -> Result<Vec<u8>> {
        if let Some(nid) = self.ec_based_nid {
            get_pk_from_sk_ec_based(sk, nid).map_err(|_| QubitCryptError::InvalidPrivateKey)
        } else if let Some(id) = self.pk_based_id {
            get_pk_from_sk_pkey_based(sk, id).map_err(|_| QubitCryptError::InvalidPrivateKey)
        } else {
