...
impl DirectoryCertificateStore {
    ...
    pub fn new(path: &str) -> Result<DirectoryCertificateStore> {
        ...
        if dir_path.exists() && dir_path.is_dir() {
            ...
            for entry in
                fs::read_dir(dir_path).map_err(|_| QubitCryptError::InvalidDirectoryPath)?
            {
                let entry = entry.map_err(|_| QubitCryptError::InvalidDirectoryPath)?;
                let path = entry.path();
                let path_str = path.to_str().ok_or(QubitCryptError::InvalidDirectoryPath)?;
            ...
            return Err(QubitCryptError::InvalidDirectoryPath);
        }
