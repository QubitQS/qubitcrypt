...
use crate::{kdf::common::kdf_type::KdfType, QubitCryptError};
...
macro_rules! derive_key {
    ($shake:expr, $ikm:expr, $info:expr, $length:expr, $salt:expr) => {{
        ...
        if let Some(s) = $salt {
            $shake.write(s).map_err(|_| QubitCryptError::KdfError)?;
        }

        // Absorb the input keying material (ikm)
        $shake.write($ikm).map_err(|_| QubitCryptError::KdfError)?;

        // Optionally absorb the context-specific information
        if $info.len() > 0 {
            $shake.write($info).map_err(|_| QubitCryptError::KdfError)?;
        }
        ...
        reader
            ...
            .map_err(|_| QubitCryptError::KdfError)?;
