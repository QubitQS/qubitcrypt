...
impl Aes {
    ...
    fn get_nonce(&self, cipher: &Cipher, nonce: Option<&[u8]>) -> Result<Vec<u8>> {
        ...
        let nonce = if let Some(nonce) = nonce {
            if nonce.len() != nonce_len {
                return Err(QubitCryptError::Unknown);
            }
            nonce.to_vec()
        } else {
            let mut nonce = vec![0u8; nonce_len];
            openssl::rand::rand_bytes(&mut nonce).map_err(|_| QubitCryptError::Unknown)?;
            nonce
        };
        Ok(nonce)
    }
    ...
    fn to_content_info(&self, ct: &[u8], nonce: &[u8], cid: Option<&str>) -> Result<Vec<u8>> {
        ...
        let parameters = der::Any::from_der(&parameters).map_err(|_| QubitCryptError::Unknown)?;
        let enc_algorithm = AlgorithmIdentifierOwned {
            oid,
            parameters: Some(parameters),
        };
        let cid = if let Some(cid) = cid {
            cid.parse().map_err(|_| QubitCryptError::InvalidOid)?
        } else {
            ID_DATA
        };

        let ct_oct_str = OctetString::new(ct.to_vec()).map_err(|_| QubitCryptError::Unknown)?;
    ...
    fn from_content_info(tag: &[u8], ct: &[u8]) -> Result<(CeaType, Vec<u8>, Vec<u8>)> {
        let eci =
            EncryptedContentInfo::from_der(ct).map_err(|_| QubitCryptError::InvalidCiphertext)?;
        let alg = eci.content_enc_alg.oid.to_string();
        let alg: CeaType = CeaType::from_oid(&alg).ok_or(QubitCryptError::InvalidCiphertext)?;
        let nonce = match alg {
            CeaType::Aes128Gcm | CeaType::Aes192Gcm | CeaType::Aes256Gcm => {
                ...
                let params = eci
                    .content_enc_alg
                    .parameters
                    .ok_or(QubitCryptError::InvalidCiphertext)?;
                let params = params
                    .to_der()
                    .map_err(|_| QubitCryptError::InvalidCiphertext)?;
...
impl Cea for Aes {
    ...
    fn key_gen(&mut self) -> Result<Vec<u8>> {
        ...
        openssl::rand::rand_bytes(&mut key).map_err(|_| QubitCryptError::Unknown)?;
        Ok(key)
    }
