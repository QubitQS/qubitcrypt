...
// Change the alias to use `QubitCryptError`.
type Result<T> = std::result::Result<T, errors::QubitCryptError>;
...
/// # Errors
///
/// `QubitCryptError::InvalidOid` will be returned if the OID is invalid
pub fn oid_to_der(oid: &str) -> Result<Vec<u8>> {
    let oid = ObjectIdentifier::new_unwrap(oid)
        .to_der()
        .map_err(|_| errors::QubitCryptError::InvalidOid)?;
    Ok(oid.to_vec())
}
